...
- Web and migration images are split `blitss` builds. Worker, beat and Temporal Django worker deliberately retain the observed locator `registry.streamloop.app/docker.io/posthog/posthog@sha256:e4431263fde4f10e935b2da9f10bd293aa4ac06ce564de21c5c8033e91af8316`; exports retains the same repository with digest `sha256:a35615349e3aa0f38050dbc06e2ccf4a70bd65d1e929e61ec5b5c6f0952b1e00`. **The repository locator is misleading:** digest metadata verifies these as `ghcr.io/blitss/posthog-worker` / `posthog-worker-exports`, OCI source `https://github.com/blitss/posthog-helm-chart`, revision `32253c7e9d511fb214ef2f8da83b29de7dde6f30`, built 2026-09-15. They are not inferred to be upstream monoliths from the locator, and their revision differs from chart `appVersion`. [`image-provenance.json`](hub-production/image-provenance.json) records the exact evidence. Preserve the working production locators; use the verified GHCR origin for direct pulls outside that mirror, not a blind prefix strip.
...
The PostgreSQL `:16` locator and the observed Keeper/Redpanda/Elasticsearch version tags are preserved, not silently retargeted. Unlike the OCI chart and digest-pinned application/ClickHouse images, these tags do not alone prove byte-identical images after registry changes. Reproduction therefore also requires retention of those exact registry artifacts; this profile is not a substitute for registry retention or database backups.
...
Requirements: Python 3 with PyYAML, `kubectl` with Kustomize, and Helm. The checker never applies resources or reconciles Flux.
