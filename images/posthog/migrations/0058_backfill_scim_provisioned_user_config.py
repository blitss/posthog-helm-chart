# Backport for PostHog 8471862b083b25d3a11b97eb7730f21aa0cb4c7f.
# https://github.com/blitss/posthog-helm-chart/issues/66
...
def _ids_to_leave_on_their_domain_key(pending: Any, config_lookup: str) -> list[Any]:
    ...
    duplicate_groups = (
        pending.values("user_id", config_lookup)
    ...
    for group in duplicate_groups:
        group_ids = list(
            pending.filter(
                ...
                **{config_lookup: group[config_lookup]},
            )
        ...
        for row_id in group_ids[1:]:
            logger.warning(
                ...
                identity_provider_config_id=str(group[config_lookup]),
            )
...
def backfill_scim_provisioned_user_config(apps: Any, schema_editor: Any) -> None:
    ...
    # posthog.1226 adds this column; 1316 only renames its historical ORM field
    # to _identity_provider_config, retaining db_column=identity_provider_config_id.
    # Resolve through that unchanged column so either valid migration order works.
    domain_config_field = next(
        field for field in OrganizationDomain._meta.fields if field.column == "identity_provider_config_id"
    )
    config_lookup = f"organization_domain__{domain_config_field.name}"
    pending = SCIMProvisionedUser.objects.using(db_alias).filter(
        identity_provider_config__isnull=True,
        **{f"{config_lookup}__isnull": False},
    )
    claimable = pending.exclude(id__in=_ids_to_leave_on_their_domain_key(pending, config_lookup))

    config_of_domain = Subquery(
        OrganizationDomain.objects.using(db_alias)
        .filter(pk=OuterRef("organization_domain_id"))
        .values(domain_config_field.attname)[:1]
    )
